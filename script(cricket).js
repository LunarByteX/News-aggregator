...
const apiUrl = `https://newsapi.org/v2/everything?q=cricket&apiKey=${apiKey}`;

const cricketApiKey = ''; // Replace with your Cricbuzz API key
        const playerRankingsApiUrl = 'https://cricbuzz-cricket.p.rapidapi.com/stats/v1/rankings/batsmen';
...
// Fetch and display Cricekt news
async function fetchcricketNews() {
  try {
    const response = await fetch(apiUrl);
    const data = await response.json();

    const articles = data.articles.slice(0, 50); // Showing top 30 news for grid
    const recommendedArticles = data.articles.slice(27, 35); // Recommend 5 articles
