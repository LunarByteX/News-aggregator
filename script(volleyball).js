...
const apiUrl = `https://newsapi.org/v2/everything?q=volleyball&apiKey=${apiKey}`;
...
// Fetch and display volleyball news
async function fetchvolleyballNews() {
  try {
    const response = await fetch(apiUrl);
    const data = await response.json();
    const articles = data.articles.slice(0, 50); // Showing top 50 news for grid
    const recommendedArticles = data.articles.slice(27, 35); // Recommend 8 articles
